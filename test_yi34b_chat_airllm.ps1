...
$Runner = Join-Path $TempDir "yi34b_chat_airllm_runner.py"
$StdoutLog = Join-Path $ReceiptsDir "yi34b_chat_console_stdout.log"
$StderrLog = Join-Path $ReceiptsDir "yi34b_chat_console_stderr.log"
...
TEXT_RECEIPT = RECEIPTS / "yi34b_chat_airllm_test.txt"
JSON_RECEIPT = RECEIPTS / "yi34b_chat_airllm_test.json"
...
def determine_airllm_compatibility(architectures: list[str], model_type: str | None) -> dict[str, Any]:
    ...
    if model_type in {"llama", "yi"}:
        return {
            "status": "uncertain",
            "mapped_class": "AirLLMLlama2",
            "dispatch": "airllm_fallback",
            "reason": f"AirLLM AutoModel fallback would try AirLLMLlama2, but architecture {first_architecture!r} does not match a direct marker.",
            "first_architecture": first_architecture,
            "model_type": model_type,
            "auto_markers": [marker for marker, _ in AIRLLM_AUTO_MARKERS],
        }
    return {
        ...
        "mapped_class": None,
        "dispatch": "none",
        "reason": "Architecture/model_type does not match AirLLM AutoModel dispatch markers.",
        "first_architecture": first_architecture,
        "model_type": model_type,
        "auto_markers": [marker for marker, _ in AIRLLM_AUTO_MARKERS],
    }
...
def write_receipts(result: dict[str, Any]) -> None:
    ...
    lines = [
        ...
        f"Position embeddings patch unchanged: {result.get('position_embeddings_patch', {}).get('present')}",
...
def main() -> int:
    ...
    result: dict[str, Any] = {
        ...
        "receipts": {
            ...
            "stdout": str(RECEIPTS / "yi34b_chat_console_stdout.log"),
            "stderr": str(RECEIPTS / "yi34b_chat_console_stderr.log"),
        },
    }

    exit_code = 1
    try:
        print(f"AirLLM Yi-34B-Chat test root: {ROOT}")
        ...
        try:
            ...
            result["airllm_architecture_compatibility"] = {"status": "no", "reason": "Config could not be loaded with trust_remote_code=True."}
        ...
        result["status"] = "PASS" if result["output_sample"].strip() else "FAIL"
        if result["status"] != "PASS":
            result["errors"].append("Generation completed but decoded output was empty.")
