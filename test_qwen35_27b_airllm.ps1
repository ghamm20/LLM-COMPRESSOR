...
$Runner = Join-Path $TempDir "qwen35_27b_airllm_runner.py"
$StdoutLog = Join-Path $ReceiptsDir "qwen35_27b_console_stdout.log"
$StderrLog = Join-Path $ReceiptsDir "qwen35_27b_console_stderr.log"
...
TEXT_RECEIPT = RECEIPTS / "qwen35_27b_airllm_test.txt"
JSON_RECEIPT = RECEIPTS / "qwen35_27b_airllm_test.json"
...
SHARD_DIR = MODELS / "Jackrong_Qwen3.5-27B-Claude-4.6-Opus-Reasoning-Distilled" / "airllm_shards"
...
AIRLLM_WEIGHT_SUFFIXES = (".safetensors", ".bin", ".pt", ".pth", ".ckpt")
...
def is_airllm_weight(name: str) -> bool:
    return name.lower().endswith(AIRLLM_WEIGHT_SUFFIXES)
...
def determine_airllm_compatibility(architectures: list[str], model_type: str | None) -> dict[str, Any]:
    ...
    if model_type in {"qwen", "qwen2"}:
        return {
            "status": "uncertain",
            "mapped_class": None,
            "reason": f"Model type {model_type!r} is Qwen-family, but AirLLM AutoModel dispatches by architecture string and this architecture does not match its current markers.",
            "first_architecture": first_architecture,
            "auto_markers": [marker for marker, _ in AIRLLM_AUTO_MARKERS],
        }
    return {
...
def metadata_preflight(token: str | None) -> dict[str, Any]:
    ...
    airllm_weights = []
    non_airllm_weights = []
    known_airllm_weight_bytes = 0
    unknown_airllm_weight_count = 0
    for sibling in siblings:
        name = file_name(sibling)
        size = file_size(sibling)
        record = {"name": name, "size_bytes": size, "size_gib": bytes_to_gib(size)}
        if is_airllm_weight(name):
            airllm_weights.append(record)
            if size is None:
                unknown_airllm_weight_count += 1
            else:
                known_airllm_weight_bytes += size
        elif is_non_airllm_weight(name):
            non_airllm_weights.append(record)

    estimated_shards_bytes = known_airllm_weight_bytes
    estimated_total_bytes = known_airllm_weight_bytes + estimated_shards_bytes
    ...
    return {
        ...
        "storage_hints": {
            ...
            "airllm_weight_file_count": len(airllm_weights),
            "unknown_airllm_weight_file_count": unknown_airllm_weight_count,
            "known_airllm_weight_bytes": known_airllm_weight_bytes,
            "estimated_weight_gib": bytes_to_gib(known_airllm_weight_bytes),
            ...
            "largest_airllm_weight_files": sorted(airllm_weights, key=lambda item: item.get("size_bytes") or 0, reverse=True)[:16],
...
def main() -> int:
    ...
    exit_code = 1
    try:
        print(f"AirLLM Qwen3.5 27B test root: {ROOT}")
        ...
        if storage.get("unknown_airllm_weight_file_count"):
            result["status"] = "PARTIAL"
            result["errors"].append("Refusing download: at least one AirLLM-compatible weight file has unknown size.")
            return 3
        if not storage.get("known_airllm_weight_bytes"):
            result["status"] = "PARTIAL"
            result["errors"].append("Refusing download: no AirLLM-compatible safetensors/bin/pt/pth/ckpt weights were found in metadata.")
            return 3
